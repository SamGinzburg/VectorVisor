...
pub struct OpenCLCWriter<'a> {
    ...
    patch_missing_builtins: bool, // AMD's OpenCL impl. leaves some basic functionality unimplemented which we
                                  // can emulate in software
}
...
impl<'a> OpenCLCWriter<'_> {
    ...
    fn emit_hypercall(
        ...
        match hypercall_id {
            ...
            WasmHypercallId::args_get => ret_str += &emit_args_get_post(self, stack_ctx, debug),
            WasmHypercallId::path_filestat_get => {
    ...
    fn emit_instructions(
        ...
        match instr {
            ...
            Instruction::I32TruncSatF64S => {
                emit_truncsat_f64(self, stack_ctx, TruncType::I32, true, debug)
            }
            Instruction::I32TruncSatF64U => {
                emit_truncsat_f64(self, stack_ctx, TruncType::I32, false, debug)
            }
            Instruction::I64TruncSatF64S => {
                emit_truncsat_f64(self, stack_ctx, TruncType::I64, true, debug)
            }
            Instruction::I64TruncSatF64U => {
                emit_truncsat_f64(self, stack_ctx, TruncType::I64, false, debug)
            }
    ...
    fn generate_function_prelude(
        ...
        if debug {
            ...
            if interleave > 0 {
                ...
                format!("local ulong2 scratch_space[{}];", 0), // not used anywhere atm
...
__attribute__((always_inline)) void {}(global uint   *stack_u32,
    ...
    fn generate_data_section(
        ...
        if debug {
            ...
            result += &format!("local ulong2 scratch_space[{}];", 0);
            // each page = 64KiB
    ...
    pub fn write_opencl_file(
        ...
        self.patch_missing_builtins = patch_missing_builtins;
        let mut output = String::new();
...
"#
        ...
        loop {
            ...
            for fastfunc in fast_function_set.iter() {
                ...
                if funcs_called.len() == 0 {
                    ...
                    let mut new_called_set: HashSet<String> =
                        fast_func_called.get(&fastfunc as &str).unwrap().clone();
        ...
        */
        ...
        for func_idx in function_idx_label.values() {
            assert!(
                kernel_partition_mappings.contains_key(&func_idx),
                format!(
                    "Assertion failure: failed to find partition for func_idx: {:?}",
                    func_idx
                )
            );
        }
