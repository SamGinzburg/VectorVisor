...
__attribute__((always_inline)) void {}(global uint   *stack_u32,
    ...
    fn zero_init_memory(&self) -> String {
        ...
        ret_str += &format!("{}{}{}",
                            ...
                            format!("\t\t{};\n", &emit_write_u32_aligned("(ulong)((global uint*)stack_u32+idx)", "(ulong)(stack_u32)", "0", "warp_idx")),
                            "\t}\n");

        // zero the heap next
        ret_str += &format!("{}{}{}",
                            "\tfor (uint idx = 0; idx < (VMM_HEAP_SIZE_BYTES / 4); idx++) {\n",
                            format!("\t\t{};\n", &emit_write_u32_aligned("(ulong)((global uint*)heap_u32+idx)", "(ulong)(heap_u32)", "0", "warp_idx")),
                            "\t}\n");
...
"#).unwrap();
        ...
        // 0 = no interleave
        write!(output, "{}", generate_read_write_calls(&self, interleave, local_work_group, mexec, debug)).unwrap();
        write!(header, "{}", generate_read_write_calls(&self, interleave, local_work_group, mexec, debug)).unwrap();
