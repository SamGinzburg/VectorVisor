...
pub fn get_called_funcs(
    ...
    (
        Vec::from_iter(HashSet::<String>::from_iter(fn_call_in_loop)),
        Vec::from_iter(HashSet::<String>::from_iter(fn_call)),
    )
}
...
pub fn form_partitions(
    writer_ctx: &OpenCLCWriter,
    num_funcs_in_partition: u32,
    instr_count_limit: u32,
    func_copy_limit: u32,
    ...
    while let Some(f_name) = func_list.pop() {
        ...
        let (loop_called_fns, called_fns) = get_called_funcs(
            ...
            func_map.get(&f_name.clone()).unwrap(),
            fastcalls,
            func_map,
            imports_map,
            &mut HashSet::new(),
        ...
        let (instr_count, _, _, _, _, _, local_size) = function_stats(
            writer_ctx,
            f_name.clone(),
            func_map.get(&f_name.clone()).unwrap(),
            fastcalls,
        ...
        for func in loop_called_fns {
            let (instr_count, _, _, _, _, _, local_size) = function_stats(
                writer_ctx,
                func.clone(),
                func_map.get(&func.clone()).unwrap(),
                fastcalls,
                func_map,
                indirect_call_mapping,
            );
        ...
        */
        ...
        for prior_partition in duplicate_funcs.iter() {
            if *prior_partition != partition_idx {
                let (prev_partition_idx, prev_partition_set) =
                    &partitions[*prior_partition as usize].clone();
                // get prev instruction count
                let (prev_instr_count, _, _, _, _, _, prev_local_size) =
                    kernel_compile_stats.get(prev_partition_idx).unwrap();
                    ...
                    && current_local_size + prev_local_size <= local_size_limit
                {
                    ...
                    partitions[*prior_partition as usize] =
                        (*prev_partition_idx, current_partition.clone());
                    let stats = (
                        current_instruction_count + prev_instr_count,
                        0,
                        0,
                        0,
                        0,
                        0,
                        current_local_size + prev_local_size,
                    );
        ...
        if !modified_prior_part {
            ...
            kernel_compile_stats.insert(
                partition_idx,
                (current_instruction_count, 0, 0, 0, 0, 0, current_local_size),
            );
            partition_idx += 1;
        } else {
            ...
            for func in prev_current_partition.iter() {
                let mut prev_partitions = dc_partitions
                    .get(&func.clone())
