...
impl<'a> StackCtx {
    ...
    pub fn emit_fastcall_header(&self) -> String {
        ...
        ret_str += &format!("global uint *heap_u32, global uint *current_mem_size, global uint *max_mem_size, global uint *globals_buffer, uint warp_idx, uint thread_idx, uint read_idx, local uchar *scratch_space) {{\n");
    ...
    pub fn emit_load_params(&self, debug_call_print: bool) -> String {
        ...
        for (local_name, local_type) in self.local_types.iter() {
            ...
            if offset < 0 {
                let param_lookup_u32 = emit_read_u32_aligned(&format!("(ulong)(stack_u32+{}+{})",
                                                          offset, 
                                                          &emit_read_u32_aligned("(ulong)(stack_frames+*sfp)", "(ulong)stack_frames", "warp_idx")),
                                                          "(ulong)stack_u32",
                                                          "warp_idx");
                let param_lookup_u64 = emit_read_u64_aligned(&format!("(ulong)(stack_u32+{}+{})",
                                                          offset, 
                                                          &emit_read_u32_aligned("(ulong)(stack_frames+*sfp)", "(ulong)stack_frames", "warp_idx")),
                                                          "(ulong)stack_u32",
                                                          "warp_idx");
    ...
    pub fn max_stack_frame_size(&self) -> usize {
        (self.i32_stack.len()*2) + (self.i64_stack.len()*2) + (self.f32_stack.len()*2) + (self.f64_stack.len()*2)
    }
        ...
        */
        ...
        if !save_intermediate_only {
            for (tmp_local, ty) in self.local_types.iter() {
                ...
                match ty {
                    StackType::i32 => {
                        ret_str += &format!("\tif (local_cache[{}]) {};\n", cache_idx, &emit_write_u32_aligned(&format!("(ulong)(stack_u32+{}+{})",
                                                                        offset,
                                                                        &emit_read_u32_aligned("(ulong)(stack_frames+*sfp)", "(ulong)stack_frames", "warp_idx")),
                                                                        "(ulong)stack_u32",
                                                                        &local,
                    ...
                    StackType::i64 => {
                        ret_str += &format!("\tif (local_cache[{}]) {};\n", cache_idx, &emit_write_u64_aligned(&format!("(ulong)(stack_u32+{}+{})",
                                                                        offset, 
                                                                        &emit_read_u32_aligned("(ulong)(stack_frames+*sfp)", "(ulong)stack_frames", "warp_idx")),
                                                                        "(ulong)stack_u32",
                                                                        &local,
                    ...
                    StackType::f32 => {
                        ...
                        ret_str += &format!("\t\t{};\n", &emit_write_u32_aligned(&format!("(ulong)(stack_u32+{}+{})",
                                                                        offset, 
                                                                        &emit_read_u32_aligned("(ulong)(stack_frames+*sfp)", "(ulong)stack_frames", "warp_idx")),
                                                                        "(ulong)stack_u32",
                                                                        "temp",
                    ...
                    StackType::f64 => {
                        ...
                        ret_str += &format!("\t\t{};\n", &emit_write_u64_aligned(&format!("(ulong)(stack_u32+{}+{})",
                                                                        offset, 
                                                                        &emit_read_u32_aligned("(ulong)(stack_frames+*sfp)", "(ulong)stack_frames", "warp_idx")),
                                                                        "(ulong)stack_u32",
                                                                        "temp",
        ...
        if !save_locals_only {
            ...
            let sfp_val = emit_read_u32_aligned("(ulong)(stack_frames+*sfp)", "(ulong)stack_frames", "warp_idx");
            ...
            for idx in i32_range {
                ...
                ret_str += &format!("\t{};\n", &emit_write_u32_aligned(&format!("(ulong)(stack_u32+{}+{}+{})", sfp_val, self.stack_frame_offset, i_name_offset),
                                                                "(ulong)stack_u32",
                                                                &i_name,
                                                                "warp_idx"));
            }
            ...
            for idx in i64_range {
                ...
                let i_name_offset = self.intermediate_offsets.get(i_name).unwrap();
                ret_str += &format!("\t{};\n", &emit_write_u64_aligned(&format!("(ulong)(stack_u32+{}+{}+{})", sfp_val, self.stack_frame_offset, i_name_offset),
                                                                "(ulong)stack_u32",
                                                                &i_name,
                                                                "warp_idx"));
            }
    ...
    pub fn restore_context(&mut self, restore_locals_only: bool, restore_intermediates_only: bool) -> String {
        ...
        if !restore_intermediates_only {
            ...
            for (tmp_local, ty) in self.local_types.iter() {
                ...
                match ty {
                    ...
                    StackType::i32 => {
                        ret_str += &format!("\tif (!local_cache[{}]) {} = {};\n", cache_idx, local, &emit_read_u32_aligned(&format!("(ulong)(stack_u32+{}+{})",
                                                                        offset, 
                                                                        &emit_read_u32_aligned("(ulong)(stack_frames+*sfp)", "(ulong)stack_frames", "warp_idx")),
                                                                        "(ulong)stack_u32",
                                                                        "warp_idx"));
                    ...
                    StackType::i64 => {
                        ret_str += &format!("\tif (!local_cache[{}]) {} = {};\n", cache_idx, local, &emit_read_u64_aligned(&format!("(ulong)(stack_u32+{}+{})",
                                                                        offset, 
                                                                        &emit_read_u32_aligned("(ulong)(stack_frames+*sfp)", "(ulong)stack_frames", "warp_idx")),
                                                                        "(ulong)stack_u32",
                                                                        "warp_idx"));
                    ...
                    StackType::f32 => {
                        ret_str += &format!("\tif (!local_cache[{}]) {{\n", cache_idx);
                        ret_str += &format!("\t\tuint temp = {};\n", &emit_read_u32_aligned(&format!("(ulong)(stack_u32+{}+{})",
                                                                                        offset, 
                                                                                        &emit_read_u32_aligned("(ulong)(stack_frames+*sfp)", "(ulong)stack_frames", "warp_idx")),
                                                                                        "(ulong)stack_u32",
                                                                                        "warp_idx"));
                    ...
                    StackType::f64 => {
                        ret_str += &format!("\tif (!local_cache[{}]) {{\n", cache_idx);
                        ret_str += &format!("\t\tulong temp = {};\n", &emit_read_u64_aligned(&format!("(ulong)(stack_u32+{}+{})",
                                                                                        offset, 
                                                                                        &emit_read_u32_aligned("(ulong)(stack_frames+*sfp)", "(ulong)stack_frames", "warp_idx")),
                                                                                        "(ulong)stack_u32",
                                                                                        "warp_idx"));
        ...
        if !restore_locals_only {
            ...
            for idx in i32_range {
                ...
                ret_str += &format!("\t{} = {};\n", &i_name,
                                                    &emit_read_u32_aligned(&format!("(ulong)(stack_u32+{}+{}+{})", sfp_val, self.stack_frame_offset, i_name_offset),
                                                                "(ulong)stack_u32",
                                                                "warp_idx"));
            }
            ...
            for idx in i64_range {
                ...
                ret_str += &format!("\t{} = {};\n", &i_name, 
                                                    &emit_read_u64_aligned(&format!("(ulong)(stack_u32+{}+{}+{})", sfp_val, self.stack_frame_offset, i_name_offset),
                                                                "(ulong)stack_u32",
                                                                "warp_idx"));
            }
     ...
     */
    pub fn restore_context_with_result_val(&mut self, restore_locals_only: bool, restore_intermediates_only: bool, ret_val_type: Option<StackType>) -> String {
