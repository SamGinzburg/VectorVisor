...
func main() {
    ...
    // Convert input size to KiB
    // 1 --> 1024
    // etc...
    if input_size < 64 {
        // 12 bytes metadata at front
        input_size = (input_size * 1024) - 12
    } else {
        // 14 bytes metadata at front
        input_size = (input_size * 1024) - 14
    }

	for i := 0; i < NUM_PARAMS; i++ {
		p := payload{Text: RandIntSlice(input_size)}
