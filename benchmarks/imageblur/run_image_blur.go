...
func RandImage(n int) string {
	// no more than 1MB
    file_data, err := ioutil.ReadFile(fmt.Sprintf("testimages/%d.png", n))
	if err != nil {
		panic(err)
	}
...
func main() {
	...
	// Debug code to write output to a file
	// pick some random image
	err = json.Unmarshal(responses[0], &m)
	if err != nil {
		panic(err)
	}
	resp := m["response"].(string)
	...
	binary_file, err := b64.StdEncoding.DecodeString(payload.Text)
	if err != nil {
		panic(err)
	}
	err = ioutil.WriteFile("test.png", binary_file, 0644)
	if err != nil {
		panic(err)
	}
}
